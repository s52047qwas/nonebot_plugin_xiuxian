...
3、灵庄取灵石：指令后加取出的金额，会先结算利息，再取出灵石石
...
async def _(bot: Bot, event: GroupMessageEvent, args: Tuple[Any, ...] = RegexGroup()):
    ...
    if mode == '存灵石':#存灵石逻辑
        if int(userinfo.stone) < num:
            msg = f"道友所拥有的灵石石为{userinfo.stone}枚，金额不足，请重新输入！"
            pic = await get_msg_pic(msg)#
            await bank.finish(MessageSegment.image(pic), at_sender=True)
            await bank.finish(f"道友所拥有的灵石石为{userinfo.stone}枚，金额不足，请重新输入！", at_sender=True)

        max = BANKLEVEL[bankinfo['banklevel']]['savemax']
        nowmax = max - bankinfo['savestone']
        
        if num > nowmax:
            msg = f"道友当前灵庄会员等级为{BANKLEVEL[bankinfo['banklevel']]['level']}，可存储的最大灵石石为{max}枚,当前已存{bankinfo['savestone']}枚灵石石，可以继续存{nowmax}枚灵石石！"
            pic = await get_msg_pic(msg)#
            await bank.finish(MessageSegment.image(pic), at_sender=True)
            await bank.finish(f"道友当前灵庄会员等级为{BANKLEVEL[bankinfo['banklevel']]['level']}，可存储的最大灵石石为{max}枚,当前已存{bankinfo['savestone']}枚灵石石，可以继续存{nowmax}枚灵石石！", at_sender=True)
        ...
        msg = f"道友本次结息时间为：{timedeff}小时，获得灵石石：{give_stone}枚!\n道友存入灵石石{num}枚，当前所拥有灵石石{userinfonowstone + give_stone}枚，灵庄存有灵石石{bankinfo['savestone']}枚"
        pic = await get_msg_pic(msg)#
        await bank.finish(MessageSegment.image(pic), at_sender=True)
        await bank.finish(f"道友本次结息时间为：{timedeff}小时，获得灵石石：{give_stone}枚!\n道友存入灵石石{num}枚，当前所拥有灵石石{userinfonowstone + give_stone}枚，灵庄存有灵石石{bankinfo['savestone']}枚", at_sender=True)

    elif mode == '取灵石':#取灵石逻辑
        if int(bankinfo['savestone']) < num:
            msg = f"道友当前灵庄所存有的灵石石为{bankinfo['savestone']}枚，金额不足，请重新输入！"
            pic = await get_msg_pic(msg)#
            await bank.finish(MessageSegment.image(pic), at_sender=True)
            await bank.finish(f"道友当前灵庄所存有的灵石石为{bankinfo['savestone']}枚，金额不足，请重新输入！", at_sender=True)
        ...
        savef(user_id, bankinfo)
        msg = f"道友本次结息时间为：{timedeff}小时，获得灵石石：{give_stone}枚!\n取出灵石石{num}枚，当前所拥有灵石石{userinfonowstone}枚，灵庄存有灵石石{bankinfo['savestone']}枚!"
        pic = await get_msg_pic(msg)#
        await bank.finish(MessageSegment.image(pic), at_sender=True)
        await bank.finish(f"道友本次结息时间为：{timedeff}小时，获得灵石石：{give_stone}枚!\n取出灵石石{num}枚，当前所拥有灵石石{userinfonowstone}枚，灵庄存有灵石石{bankinfo['savestone']}枚!", at_sender=True)
        ...
        if userlevel == str(len(BANKLEVEL)):
            ...
            pic = await get_msg_pic(msg)#
            await bank.finish(MessageSegment.image(pic), at_sender=True)
            await bank.finish(f"道友已经是本灵庄最大的会员啦！", at_sender=True)
        
        stonecost = BANKLEVEL[f'{int(userlevel)}']['levelup']
        if int(userinfo.stone) < stonecost:
            msg = f"道友所拥有的灵石石为{userinfo.stone}枚，当前升级会员等级需求灵石石{stonecost}枚金额不足，请重新输入！"
            pic = await get_msg_pic(msg)#
            await bank.finish(MessageSegment.image(pic), at_sender=True)
            await bank.finish(f"道友所拥有的灵石石为{userinfo.stone}枚，当前升级会员等级需求灵石石{stonecost}枚金额不足，请重新输入！", at_sender=True)
        
        sql_message.update_ls(user_id, stonecost, 2)
        bankinfo['banklevel'] = f'{int(userlevel) + 1}'
        savef(user_id, bankinfo)
        msg = f"道友成功升级灵庄会员等级，消耗灵石石{stonecost}枚，当前为：{BANKLEVEL[f'{int(userlevel) + 1}']['level']}，灵庄可存有灵石石上限{BANKLEVEL[f'{int(userlevel) + 1}']['savemax']}枚"
        pic = await get_msg_pic(msg)#
        await bank.finish(MessageSegment.image(pic), at_sender=True)
        await bank.finish(f"道友成功升级灵庄会员等级，消耗灵石石{stonecost}枚，当前为：{BANKLEVEL[f'{int(userlevel) + 1}']['level']}，灵庄可存有灵石石上限{BANKLEVEL[f'{int(userlevel) + 1}']['savemax']}枚", at_sender=True)
    
    elif mode == '信息':#查询灵庄信息
        msg = f'''
道友的灵庄信息：
已存：{bankinfo['savestone']}灵石石
存入时间：{bankinfo['savetime']}
灵庄会员等级：{BANKLEVEL[bankinfo['banklevel']]['level']}
当前拥有灵石石：{userinfo.stone}
当前等级存储灵石石上限：{BANKLEVEL[bankinfo['banklevel']]['savemax']}枚
                        '''
        pic = await get_msg_pic(msg)#
        await bank.finish(MessageSegment.image(pic), at_sender=True)
        await bank.finish(f'''
...
当前等级存储灵石石上限：{BANKLEVEL[bankinfo['banklevel']]['savemax']}枚
        ...
        pic = await get_msg_pic(msg)#
        await bank.finish(MessageSegment.image(pic), at_sender=True)
        await bank.finish(f'道友本次结息时间为：{timedeff}小时，获得灵石石：{give_stone}枚！', at_sender=True)
