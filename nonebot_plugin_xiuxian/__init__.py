...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if message == "排行榜" or message == "修仙排行榜" or message == "境界排行榜":
        ...
        if XiuConfig().img:
            msg = await pic_msg_format(p_rank, event)
            pic = await get_msg_pic(msg)
        ...
        if XiuConfig().img:
            msg = await pic_msg_format(a_rank, event)
            pic = await get_msg_pic(msg)
            await rank.finish(MessageSegment.image(pic))
        else:
        ...
        if XiuConfig().img:
            msg = await pic_msg_format(s_rank, event)
            pic = await get_msg_pic(msg)
            await rank.finish(MessageSegment.image(pic))
        else:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if XiuConfig().img:
        mes = await pic_msg_format(mes, event)
...
def perform_breakthrough(user_id: str, exp: int, stone: int,need_exp: int, need_stone: int,):
    # 编写突破后的处理逻辑，例如更新用户的修为和灵石，以下为示例
    exp = exp - need_exp
    stone = stone - need_stone
    return exp, stone
...
async def _(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if "启用" in group_msg:
        ...
        msg = "当前群聊修仙模组已启用！"
        if XiuConfig().img:
            result = await pic_msg_format(msg,event)
